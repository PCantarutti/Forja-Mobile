...
type Destino = "galeria" | "pasta" | "compartilhar";
...
async function salva(paths: string[], destino: Destino): Promise<string> {
  if (destino === "compartilhar") {
    const arq = await baixaLocal(paths[0]);
    await Sharing.shareAsync(arq.uri, { mimeType: "image/png", dialogTitle: "Compartilhar imagem" });
    return "";
  }
  if (destino === "pasta") {
    ...
    for (const p of paths) {
      ...
      dir.createFile(arq.name, "image/png").write(await arq.bytes());
      arq.delete();
    }
    return `${paths.length === 1 ? "A imagem foi salva" : `${paths.length} imagens foram salvas`} na pasta escolhida.`;
  }
  ...
  return `${paths.length === 1 ? "A imagem foi salva" : `${paths.length} imagens foram salvas`} na galeria (pasta DCIM, junto das fotos da câmera).`;
}

/** Imagens (stable-diffusion.cpp local): lotes da conversa + caixa de prompt com os ajustes do desktop. */
export default function Imagens({ conv, onCriada, onTurno }: { conv: Conv | null; onCriada: (c: Conv) => void; onTurno: () => void }) {
  ...
  return (
    <View style={{ flex: 1, paddingBottom: teclado }}>
      ...
      <Visor img={ver} onFecha={() => setVer(null)}
             onEditar={(p) => { setRefs((x) => (x.includes(p) || x.length >= MAX_REFS ? x : [...x, p])); setVer(null); }}
             onSemente={(n) => { muda({ seed: n, seed_mode: "fixa" }); setVer(null); }} onBaixar={baixa} />
    </View>
  );
}
...
function Visor({ img, onFecha, onEditar, onSemente, onBaixar }:
      ...
      <View style={{ flex: 1, backgroundColor: "#000" }}>
        <Pressable style={{ flex: 1 }} onPress={onFecha}>
          <ScrollView maximumZoomScale={4} minimumZoomScale={1} contentContainerStyle={{ flexGrow: 1, justifyContent: "center" }}>
            <Image source={{ uri: urlImagem(img.path) }} style={{ width: "100%", aspectRatio: proporcao }} resizeMode="contain"
        ...
        <View style={{ padding: 14, paddingBottom: inset.bottom + 14, gap: 10 }}>
          <Text style={{ color: c.muted, fontFamily: mono, fontSize: 12, textAlign: "center" }}>{img.model_name} · semente {img.seed}</Text>
          <View style={{ flexDirection: "row", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <Pressable style={s.btnSec} onPress={() => onEditar(img.path)}><Text style={s.btnSecTxt}>Editar a partir desta</Text></Pressable>
            <Pressable style={s.btnSec} onPress={() => onSemente(img.seed)}><Text style={s.btnSecTxt}>Usar esta semente</Text></Pressable>
          </View>
        </View>
      </View>
    </Modal>
